...
import { routeConverter } from './router.ts'
import { dialogExecutor, routeExecutor, storeExecutor } from './executor.ts'
import { DialogNotFound } from './exceptions/index.ts'
...
class Bot<T> implements IBot {
	...
	async sendMessage(activity: IActivity): Promise<void> {
		...
		try {
			if (dialog) {
				const response = await dialogExecutor(
					this.store,
					activity,
					dialog,
				)

				const store = storeExecutor(
					this.store,
					response.state || {},
					response.navigateTo || this.store.startRoute,
				)
				this.setStore(store)
